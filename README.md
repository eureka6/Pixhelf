...
缩略图缓存在 `./.pixhelf-cache/thumbnails` 下的单一版本化目录，原图不会被修改。点击图片后，查看器会先显示这份缩略图，再加载原图，并提前预取相邻原图；超大原图会在浏览器内解码到 GPU 安全画布，不会生成第二套预览缓存。查看器支持键盘切换、滚轮缩放，以及移动端滑动、双击和双指缩放。缓存目录必须位于图库目录之外。
