...
缩略图缓存在 `./.pixhelf-cache/thumbnails` 下的单一版本化目录，原图不会被修改。查看器拥有独立于瀑布流可见性的资源队列：提前解码切图窗口内的缩略图并预取相邻原图，切换目标时立即显示对应缩略图，原图下载且解码完成后再无闪烁替换；桌面端会直接复用已就绪的常规原图，移动端切图首帧先使用与视口及 DPR 匹配的高质量位图，当前图片稳定后在后台自动加载真实原图，放大时立即切换，复位后回到更适合屏幕采样的视口位图，连续切图会取消过期升级任务。只有超过安全边界的极端大图才绘制到 GPU 安全画布。查看器支持键盘方向键、鼠标前进/后退侧键和滚轮缩放，以及移动端滑动、双击和双指缩放；在适应屏幕状态下继续向下滚动或上滑，可从图片首屏连续进入详情第二屏。关闭查看器时会在瀑布流布局稳定后把当前图片动画送回对应卡片，即使期间发生续载或窗口尺寸变化也会重新锁定落点。窗口宽度引发瀑布流重排时，会按图片 ID 保持当前焦点或视口中心图片的相对位置。缓存目录必须位于图库目录之外。
