...
- **实况照片**：支持 JPEG、PNG、WebP 与同名 MOV／MP4／M4V 配对，以及 Google／Samsung 二合一 JPG。桌面悬停播放；手机首次轻点播放，之后再点打开大图，播完也有效；播放过另一张实况后，回到之前的图片需先点一次播放。大图自动播放一遍，之后单击重播。全页面最多同时播放一个，等待时显示细进度条。浏览器可缓存原视频，不转码、不生成服务端视频缓存文件，也不依赖 FFmpeg。HEVC 需浏览器和设备支持；HEIC／HEIF 照片暂不支持，可导出为 JPEG 并保留同名视频。
